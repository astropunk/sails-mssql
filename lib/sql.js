...
var sql = {
	...
	serializeOptions: function(collectionName, options) {
		...
		if (options.criteria.where) {
			queryPart += ' WHERE ' + sql.where(collectionName, options.criteria.where) + ' ';
		}

		if (options.criteria.groupBy) {
			queryPart += 'GROUP BY ';

			// Normalize to array
			if(!Array.isArray(options.criteria.groupBy)) options.criteria.groupBy = [options.criteria.groupBy];
			options.criteria.groupBy.forEach(function(key) {
				queryPart += key + ', ';
			});
		...
		if (options.criteria.sort.length > 0 && !options.criteria.skip) {
			queryPart += 'ORDER BY ';

			// Sort through each sort attribute criteria
			_.each(options.criteria.sort, function(direction, attrName) {
