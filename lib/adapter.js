...
module.exports = (function () {
  ...
  var adapter = {
    ...
    // Check if this is an aggregate query and that there is something to return
      if (options && (options.groupBy || options.sum || options.average || options.min || options.max)) {
      ...
      var schemaName = getSchemaName(connection, collection);
      var tableName =  '[' + schemaName + ']' + '.[' + collection.using + ']';
      var statement = sql.selectQuery(tableName, options);
      adapter.connectConnection(connection, (err, uniqId) => {
        if (err) {
          console.error(err);
          return cb(err);
        }
        ...
        if (!uniqId) {
          ...
          mssqlConnect = connections[connection].mssqlConnection[uniqId.uniqId];
        }

        var request = new mssql.Request(mssqlConnect);
        request.query(statement, (err, recordset) => {
          if (err) return cb(err);
          if (!connections[connection].config.persistent) {
            mssqlConnect && mssqlConnect.close();
          }
    ...
    sendNativeQuery: function (connection, query, data, cb) {
      ...
      adapter.connectConnection(connection, function __FIND__(err, uniqId) {
        ...
        request.query(query, function (err, recordset) {
          if (err) return cb(err);
          if (connections[connection] && !connections[connection].config.persistent) {
            mssqlConnect && mssqlConnect.close();
          }
    ...
    getPrimaryKey: function (connection, collection) {
      var pk = 'id';
      Object.keys(connections[connection].collections[collection.using].definition).forEach(function(key) {
        if(!connections[connection].collections[collection.using].definition[key].hasOwnProperty('primaryKey')) return;
        pk = key;
      });
      return pk;
    },
  ...
  function getSchemaName (connection, collection) {
    var collectionObject = connections[connection].collections[collection.using];
    if (!collectionObject.hasOwnProperty('meta')) return 'dbo';
